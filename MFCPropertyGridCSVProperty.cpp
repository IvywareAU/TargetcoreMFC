...
IMPLEMENT_DYNAMIC(CMFCPropertyGridCSVProperty, CMFCPropertyGridProperty)

CMFCPropertyGridCSVProperty::CMFCPropertyGridCSVProperty ( const CString& strName, LPCTSTR lpszCSV
                                                      ...
                                                      //, m_dwPenDialogFlags(dwPenDialogFlags)
{
    ...
    //m_bIsValueList   = TRUE;
    m_strCSV         = lpszCSV;
    m_strCSVorig     = lpszCSV;
    //m_oLOGPENorig    = oLOGPEN;
    m_bAllowEdit     = TRUE; //FALSE;
    //m_Color          = oLOGPEN.lopnColor; //color;
}

CMFCPropertyGridCSVProperty::~CMFCPropertyGridCSVProperty()
{
}
void
CMFCPropertyGridCSVProperty::OnDrawValue ( CDC *pDC, CRect rect )
{
AllowEdit();
...
void
CMFCPropertyGridCSVProperty::OnClickButton(CPoint /*point*/)
{
    ASSERT_VALID(this);
...
#pragma warning(disable : 4244)
    CSVDialog dlg ( m_strCSV, 0, NULL, m_pWndList );
...
CString
CMFCPropertyGridCSVProperty::FormatProperty ( )
{
    CString str;
    ...
    if (nLogY != 0)
    {
      str.Format ( L"%s\0", (LPCTSTR)m_strCSV );
    }
    else
    {
...
void
CMFCPropertyGridCSVProperty::ResetOriginalValue()
{
    CMFCPropertyGridProperty::ResetOriginalValue();
    m_strCSV = m_strCSVorig;
}
BOOL
CMFCPropertyGridCSVProperty::IsValueChanged() const
{
    return (m_strCSV == m_strCSVorig) ? FALSE : TRUE;
}
