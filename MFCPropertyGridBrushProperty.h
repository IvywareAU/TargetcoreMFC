...
//       : Intention is to simply extend MFC with CMFCPropertyGridBrushProperty class
...
class CMFCPropertyGridBrushProperty : public CMFCPropertyGridProperty
{
    DECLARE_DYNAMIC(CMFCPropertyGridBrushProperty)

    // Construction
    public:
	      CMFCPropertyGridBrushProperty ( const CString& strName, LOGBRUSHcrx& oLOGBRUSHcrx
                                      , DWORD dwBrushDialogFlags = CF_EFFECTS | CF_SCREENFONTS
                                      , LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0, COLORREF color = (COLORREF)-1);
      virtual
       ~CMFCPropertyGridBrushProperty();
    ...
    // Attributes
    public:
	    LPLOGBRUSH GetLogBrush() { return &m_oLOGBRUSHcrx; }
	    COLORREF   GetColor() const { return m_Color; }

    protected:
	    LOGBRUSHcrx  m_oLOGBRUSHcrx;
	    LOGBRUSHcrx  m_oLOGBRUSHorig;
	    DWORD_PTR    m_dwBrushDialogFlags;
	    COLORREF     m_Color;
