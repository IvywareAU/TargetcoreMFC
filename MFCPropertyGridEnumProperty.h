...
class TargetCoreMFC_EXT CMFCPropertyGridEnumProperty : public CMFCPropertyGridProperty
{
    DECLARE_DYNAMIC(CMFCPropertyGridEnumProperty)

    // Construction
    public:
	      CMFCPropertyGridEnumProperty ( const CString& strName, int eNum
                                     , DWORD dwFontDialogFlags = CF_EFFECTS | CF_SCREENFONTS
                                     , LPCTSTR lpszDescr = NULL, DWORD_PTR dwData = 0, COLORREF color = (COLORREF)-1);
      virtual
       ~CMFCPropertyGridEnumProperty();

    // Operations
    public:
      int
        AddOptionEnum ( LPCTSTR lpszItem, DWORD dwEnumValue );
      DWORD
    ...
    public:
      ...
      CMap_String_DWORD m_oCMapEnumOptions;
