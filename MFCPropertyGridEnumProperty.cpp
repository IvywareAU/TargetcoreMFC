...
//
//
//  CMFCPropertyGridEnumProperty implementations.
...
IMPLEMENT_DYNAMIC(CMFCPropertyGridEnumProperty, CMFCPropertyGridProperty)

CMFCPropertyGridEnumProperty::CMFCPropertyGridEnumProperty ( const CString& strName, int dwEnumValue
...
CMFCPropertyGridEnumProperty::~CMFCPropertyGridEnumProperty()
{
}
...
int
CMFCPropertyGridEnumProperty::AddOptionEnum ( LPCTSTR lpszOption, DWORD dwEnumValue )
{

    AddOption ( lpszOption );
    //m_dwBMaskOptions |= dwBMaskOption;
    m_oCMapEnumOptions.SetAt ( lpszOption, dwEnumValue );
    return 0;
}

DWORD
CMFCPropertyGridEnumProperty::SetBMask ( DWORD dwEnumValue )
{
    m_dwEnumValue  = dwEnumValue;
    //_variant_t var       = GetValue ( );
    POSITION pos = m_oCMapEnumOptions.GetStartPosition();
    while ( pos )
    {
      CString strOption;
      DWORD    dwOptionValue;
      m_oCMapEnumOptions.GetNextAssoc ( pos, strOption, dwOptionValue );
      if ( dwEnumValue == dwOptionValue )
      {
        SetValue ( strOption );
        return dwEnumValue;
      }
...
DWORD
CMFCPropertyGridEnumProperty::GetBMask ( )
{
   _variant_t var = GetValue ( );
    ...
    while ( pos )
    {
      ...
      if ( strSelected == strOption )
        return dwOptionValue;
    }
    
    // Tidy up, and
    return m_dwEnumValue;
}

void
CMFCPropertyGridEnumProperty::OnDrawValue_ ( CDC *pDC, CRect rect )
{
//AllowEdit();
...
void
CMFCPropertyGridEnumProperty::OnClickButton_(CPoint /*point*/)
{
    ASSERT_VALID(this);
...
CString
CMFCPropertyGridEnumProperty::FormatProperty_ ( )
{
    CString str;
...
void
CMFCPropertyGridEnumProperty::ResetOriginalValue_()
{
    CMFCPropertyGridProperty::ResetOriginalValue();
    m_oLOGPEN = m_oLOGPENorig;
}
BOOL
CMFCPropertyGridEnumProperty::IsValueChanged_() const
{
