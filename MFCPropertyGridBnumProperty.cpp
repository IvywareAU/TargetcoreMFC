...
IMPLEMENT_DYNAMIC(CMFCPropertyGridBnumProperty, CMFCPropertyGridProperty)

CMFCPropertyGridBnumProperty::CMFCPropertyGridBnumProperty ( const CString& strName, int eNum
                                 , DWORD dwPenDialogFlags, LPCTSTR lpszDescr, DWORD_PTR dwData, COLORREF color)
                            : CMFCPropertyGridProperty ( strName, COleVariant(L""), lpszDescr, dwData)
                            , m_dwPenDialogFlags(dwPenDialogFlags), m_Color(color)
{
    //m_dwFlags        = AFX_PROP_HAS_BUTTON;
    //m_bIsValueList   = TRUE;
    //m_oLOGPENcrx        = oLOGPEN;
...
CMFCPropertyGridBnumProperty::~CMFCPropertyGridBnumProperty()
{
}
...
int
CMFCPropertyGridBnumProperty::AddOptionBMask ( LPCTSTR lpszOption, DWORD dwBMaskOption )
{

    AddOption ( lpszOption );
    m_dwBMaskOptions |= dwBMaskOption;
    m_oCMapOptions.SetAt ( lpszOption, dwBMaskOption );
    return 0;
}

DWORD
CMFCPropertyGridBnumProperty::SetBMask ( DWORD dwBMask )
{
    m_dwBMask  = ~m_dwBMaskOptions & dwBMask;
      dwBMask &=  m_dwBMaskOptions;    // Isolate bits of interest
    //_variant_t var       = GetValue ( );
    POSITION pos = m_oCMapOptions.GetStartPosition();
    while ( pos )
    {
      CString strOption;
      DWORD    dwBMaskOption;
      m_oCMapOptions.GetNextAssoc ( pos, strOption, dwBMaskOption );
      if ( dwBMask == dwBMaskOption )
      {
        SetValue ( strOption );
        return dwBMaskOption;
      }
    }
...
DWORD
CMFCPropertyGridBnumProperty::GetBMask ( )
{
   _variant_t var = GetValue ( );
    CString   strSelected = var;
    POSITION  pos = m_oCMapOptions.GetStartPosition();
    while ( pos )
    {
      CString strOption;
      DWORD    dwBMaskOption;
      m_oCMapOptions.GetNextAssoc ( pos, strOption, dwBMaskOption );
      if ( strSelected == strOption )
        return m_dwBMask | dwBMaskOption;
    }
...
void
CMFCPropertyGridBnumProperty::OnDrawValue_ ( CDC *pDC, CRect rect )
{
//AllowEdit();
...
void
CMFCPropertyGridBnumProperty::OnClickButton_(CPoint /*point*/)
{
    ASSERT_VALID(this);
...
#pragma warning(disable : 4244)
    CPenDialog dlg ( &m_oLOGPEN, m_dwPenDialogFlags, NULL, m_pWndList );
#pragma warning(default : 4244)
    ...
    if (dlg.DoModal() == IDOK)
    {
      memcpy ( &m_oLOGPEN, dlg.GetCurrentPen(), sizeof(m_oLOGPEN) );
...
CString
CMFCPropertyGridBnumProperty::FormatProperty_ ( )
{
    CString str;
    ...
    if (nLogY != 0)
    {
      str.Format ( _T("%i-"), m_oLOGPEN.lopnWidth.x );
...
void
CMFCPropertyGridBnumProperty::ResetOriginalValue_()
{
    CMFCPropertyGridProperty::ResetOriginalValue();
    m_oLOGPEN = m_oLOGPENorig;
}
BOOL
CMFCPropertyGridBnumProperty::IsValueChanged_() const
{
    return (memcmp(&m_oLOGPEN,&m_oLOGPENorig,sizeof(LOGPEN)) != 0 );
