...
// Construction
CInPlaceEdit::CInPlaceEdit(int iItem, int iSubItem, CString sInitText, DWORD dwWM_APP_LISTCTRL_IPEDIT ):m_sInitText( sInitText )
{	
    m_iItem    = iItem;
    m_iSubItem = iSubItem;
    m_dwWM_APP_LISTCTRL_IPEDIT = dwWM_APP_LISTCTRL_IPEDIT;	
    m_bESC     = FALSE;
} 
...
{
    // Abandon editing
    // NOTES: Changes still locked in this instance, no need
    //        to reset	
    ...
    // Termination of field edit
    // NOTES: Perform single complete notification at end of edit
    //      : Build and send LV_DISPINFO structure
    if ( nChar == VK_RETURN )
    {
      ...
      {
        ASSERT(m_dwWM_APP_LISTCTRL_IPEDIT);
        pWndViewAttachmentsDlg->SendMessage( m_dwWM_APP_LISTCTRL_IPEDIT, 
    ...
    // Progressive inplace edit housekeeping 
    CString str; GetWindowText( str );	
    CWindowDC dc(this);	
